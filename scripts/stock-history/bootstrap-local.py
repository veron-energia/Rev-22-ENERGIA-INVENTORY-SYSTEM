"""Rebuild the complete schema in the exclusive local Stock History fixture.
...
DB = 'energia_stock_history_test'
if os.environ.get('PGDATABASE') != DB or os.environ.get('PGHOST') != '/tmp' or os.environ.get('PGPORT') != '55443':
...
if pathlib.Path(r.stdout.strip()).resolve() != pathlib.Path('.stock-history-test/data').resolve():
...
logs = pathlib.Path('.stock-history-test')
