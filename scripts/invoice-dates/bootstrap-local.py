"""Rebuild the complete schema in the exclusive local Invoice Date fixture.
...
DB = 'energia_invoice_date_test'
if os.environ.get('PGDATABASE') != DB or os.environ.get('PGHOST') != '/tmp' or os.environ.get('PGPORT') != '55445':
...
if pathlib.Path(r.stdout.strip()).resolve() != pathlib.Path('.invoice-date-test/data').resolve():
...
logs = pathlib.Path('.invoice-date-test')
