...
export function documentImageBlob(d: PdfDoc): Promise<Blob> {
  ...
  font(2.4); x.fillStyle = GREY;
  x.fillText('Wellness & Retail', M, y + 3.4 * MM);

  x.fillStyle = BLACK; font(4, 'bold');
  ...
  for (const t of [d.kindLabel, d.storeName, d.storeAddress,
    ...
    if (!t) continue;
    for (const ln of wrap(String(t), 80 * MM)) { right(ln, RIGHT, hy); hy += 3.1 * MM; }
  }
  ...
  // ---- Customer -----------------------------------------------------
  font(2.4, 'bold'); x.fillStyle = GREY;
  x.fillText('BILL TO', M, y); y += 4 * MM;
  font(3.2); x.fillStyle = BLACK;
  x.fillText(d.customerName, M, y); y += 3.6 * MM;
  if (d.customerContact) {
    font(2.4); x.fillStyle = GREY; x.fillText(d.customerContact, M, y); y += 3.6 * MM;
  }
  y += 2 * MM;
  ...
  const drawLine = (l: PdfLine) => {
    font(2.8); x.fillStyle = BLACK;
    const nameLines = wrap(l.name, cQty - M - 3 * MM);
    let ny = y;
    for (const ln of nameLines) { x.fillText(ln, M, ny); ny += 3.4 * MM; }
    right(String(l.qty), cQty, y);
    right(money(l.unit), cUnit, y);
    font(2.8, 'bold'); right(money(l.total), cTot, y); font(2.8);
    for (const n of l.notes ?? []) {
      font(2.1); x.fillStyle = GREY;
      x.fillText(n, M + 2 * MM, ny); ny += 3 * MM;
      font(2.8); x.fillStyle = BLACK;
    }
  ...
  if (d.grandTotal) {
    ...
    font(3.7, 'bold'); x.fillStyle = BLACK;
  ...
  if (d.payments?.length) {
    ...
    font(2.8); x.fillStyle = BLACK;
    for (const [k, v] of d.payments) { x.fillText(k, M, y); right(v, cTot, y); y += 3.8 * MM; }
  }

  // ---- Signatures ---------------------------------------------------
  y = Math.max(y + 4 * MM, H - 48 * MM);
  const colW = (RIGHT - M - 6 * MM) / 2;
  if (d.staffName) { font(3, 'bold italic'); x.fillStyle = BLACK; x.fillText(d.staffName, M, y); }
  ...
  for (const ln of wrap(
    ...
    RIGHT - M)) { x.fillText(ln, M, y); y += 2.6 * MM; }

  if (d.payDetails?.length) {
    y += 1.4 * MM;
    font(2.1, 'bold'); x.fillStyle = BLACK; x.fillText('How to pay', M, y);
    font(2.1); x.fillStyle = '#333333';
    let px = M + 15 * MM;
  ...
  if (d.footerBits?.length) {
    ...
    y += 2.8 * MM;
    font(1.9); x.fillStyle = GREY; x.textAlign = 'center';
    for (const ln of wrap(d.footerBits.join('  |  '), RIGHT - M)) { x.fillText(ln, W / 2, y); y += 2.4 * MM; }
    x.textAlign = 'left';
  }
