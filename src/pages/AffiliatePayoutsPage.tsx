...
const AffiliatePayoutsPage: React.FC = () => {
  ...
  const badge = (s: string) => <span className={'badge ' + (s === 'paid' ? 'badge-success' : 'badge-muted')}>{s === 'paid' ? 'Paid' : 'Cancelled'}</span>;
        ...
        <>
          ...
          <MobileCards>
            {rows.length === 0
              ...
              : rows.map((r, i) => (
                <MCard key={i} title={r.payout_month}
                  rows={[['Tier 1', money(r.tier1)], ['Tier 2', money(r.tier2)], ['Total', money(r.total)],
                         ['Paid Date', dateStr(r.paid_date)], ['Status', badge(r.status)]]} />
              ))}
          </MobileCards>
        </>
      )}
